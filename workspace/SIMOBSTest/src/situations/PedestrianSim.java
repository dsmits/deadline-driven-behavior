...
public class PedestrianSim implements Runnable {
	...
	public PedestrianSim(String configFile) {
		...
		// TODO: Make port configurable
		try {
			listener = new ServerSocket(DEFAULT_PORT, 0,
	...
	public void start() {
		...
		try {
			while (!isKilled()) {
				...
				System.out.println("Server inetaddress: "
						+ listener.getInetAddress());
				server = listener.accept();
				System.out.println("Connected with pedestrian");
				threadPool.execute(new SimulationConnectionHandler(server,
						pedestrianManager));
	...
	public class SimulationConnectionHandler implements Runnable {
		...
		private SimulationConnectionHandler(Socket server,
		...
		/**
		 * Handles a single request from a single pedestrian and sends back an
		 * action to the pedestrian.
		 */
		public void handlePedestrian() {
			...
			try {
				...
				/*
				 * PedestrianManager pedestrianManager = (PedestrianManager)
				 * context .get("pedestrianManager"); SituationManager
				 * situationManager = (SituationManager) context
				 * .get("situationManager"); PetriNetManager petriNetManager =
				 * (PetriNetManager) context.get("petriNetManager");
				 * 
				 * List<ISituation> currentSituations =
				 * situationManager.getSituations( observation.getLocationX(),
				 * observation.getLocationY());
				 */
				/*
				 * pedestrianManager
				 * .updateSituations(observation.getPedestrianId(),
				 * currentSituations);
				 */
		...
		/*
		 * private InputStream checkInput(InputStream in) { BufferedReader
		 * reader = new BufferedReader(new InputStreamReader(in)); InputStream
		 * newIn = null; try { String line = reader.readLine();
		 * System.out.println("Line from input: " + line); newIn = new
		 * ByteArrayInputStream(line.getBytes("UTF-8")); } catch (IOException e)
	...
	/*
	 * private class SimulationConnectionReceiver implements
	 * IConnectionReceiver{
	 * 
	 * //private Socket server; private Map<String, Object> context;
	 * 
	 * private SimulationConnectionReceiver(Map<String, Object> context){
	 ...
	 * protobuf.ObservationMessage.Observation.parseFrom(data);
	 * PedestrianManager pedestrianManager =
	 * (PedestrianManager)context.get("pedestrianManager"); SituationManager
	 * situationManager = (SituationManager)context.get("situationManager");
	 * List<ISituation> currentSituations =
	 * situationManager.getSituations(observation.getLocationX(),
	 * observation.getLocationY());
	 * pedestrianManager.updateSituations(observation
	 * .getPedestrianId(),currentSituations); Action action =
	 * buildAction(pedestrianManager.step(observation));
	 * 
	 * // Send response back to client
	 * //action.writeTo(server.getOutputStream());
	 * 
	 * System.out.println("Done handling connection with pedestrian");
	 * 
	 ...
	 * }
	 * 
	 * private Action buildAction(String actionDescription){ Action.Builder
	 * action = Action.newBuilder(); action.setAction(actionDescription); return
	 * action.build(); }
	 * 
	 * }
	 */
